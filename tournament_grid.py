...
def create_grid(users, cat_name, comp_name, city, members, tyli=False):
    ...
    if len(users) > 1:
        ...
        # Места и судьи
        cell = sheet.cell(row=sheet.max_row + 2, column=last_column - 1)
        cell.border = bot_border
        ...
        for i, text in enumerate(members.keys()):
            ...
            cell.border = bot_border
            cell2 = sheet.cell(row=last_row, column=last_column + 2, value=f'/{members[text]}')
            last_row += 2
            cell2.border = bot_border
            cell2.alignment = right
    elif len(users) == 1:
        cell = sheet.cell(row=5, column=last_column, value=users[0])
        cell.border = bot_border
        cell = sheet.cell(row=sheet.max_row + 1, column=last_column)
        cell.border = right_border
        cell = sheet.cell(row=sheet.max_row, column=last_column + 1)
        cell.border = bot_border
        cell = sheet.cell(row=sheet.max_row + 1, column=last_column)
        cell.border = bot_right_border

        cell = sheet.cell(row=sheet.max_row + 2, column=last_column)
        cell.border = bot_border
        cell = sheet.cell(row=sheet.max_row + 1, column=last_column)
        cell.border = right_border
        cell = sheet.cell(row=sheet.max_row, column=last_column + 1)
        ...
        cell = sheet.cell(row=sheet.max_row + 1, column=last_column)
        cell.border = bot_right_border
        cell = sheet.cell(row=sheet.max_row, column=last_column + 1, value='3 место')
        cell.alignment = center
        last_row = sheet.max_row + 2
        winners = ['1 место:', '2 место:', '3 место:']
        for i, text in enumerate(members.keys()):
