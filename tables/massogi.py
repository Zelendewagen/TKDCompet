...
class MassogiFrame(tk.Frame):
    ...
    def create_category_table(self):
        self.category_table = self.create_table(
            ...
            headers=['№', 'Пол', 'Возраст', 'Категория', 'Вес ', 'Участников', 'Совпадений', 'ID'])
    ...
    def create_unallocated_table(self):
        self.unallocated_table = self.create_table(
            ...
            headers=['№', 'ФИО', 'Полных лет', 'Категория', 'Вес', 'Город', 'Тренер', 'ID', ],
            padx=10)

    def create_athletes_table(self):
        self.athletes_table = self.create_table(
            label_text="Участники",
            headers=['ID', 'ФИО', 'Полных лет', 'Категория', 'Вес', 'Город', 'Тренер'])
    ...
    def open_add_change_category_window(self, change=False, compet_id=None):
        ...
        self.top_window.geometry(
            config.geometry(self.master, config.massogi_window_width, config.massogi_window_height))
        ...
        entries[1].bind("<<ComboboxSelected>>", lambda x: self.update_entry_counts(entries[1], entries[2]))
        entries[2].grid(row=1, column=3, sticky="w", pady=(0, 10))

        tk.Label(frame, text="Категория от:").grid(row=2, column=0, sticky="w", pady=(0, 10))
        tk.Label(frame, text="до:").grid(row=2, column=2, sticky="w", pady=(0, 10), padx=(10, 0))
        entries[3].grid(row=2, column=1, sticky="w", pady=(0, 10))
        entries[3].bind("<<ComboboxSelected>>", lambda x: self.update_entry_categories(entries[3], entries[4]))
        entries[4].grid(row=2, column=3, sticky="w", pady=(0, 10))

        tk.Label(frame, text="Вес от:").grid(row=3, column=0, sticky="w")
        tk.Label(frame, text="до:").grid(row=3, column=2, sticky="w", padx=(10, 0))
        entries[5].grid(row=3, column=1, sticky="w")
        entries[5].bind("<<ComboboxSelected>>", lambda x: self.update_entry_counts(entries[5], entries[6]))
        entries[6].grid(row=3, column=3, sticky="w")

        buttons_frame = tk.Frame(frame)
        buttons_frame.grid(columnspan=4, row=4, pady=20, sticky="ew")

        if change:
            with sqlite3.connect(DB_FILE) as conn:
                cursor = conn.cursor()
                cursor.execute("SELECT * FROM massogi WHERE id =?", (compet_id,))
    ...
    def add_change_category(self, entries, cat_id=None, change=False):
        ...
        with sqlite3.connect(DB_FILE) as conn:
            ...
            if change:
                parameters += (cat_id,)
                cursor.execute(
                    "UPDATE massogi SET gender=?, age_form=?, age_to=?, category_from=?, category_to=?, weight_from=?, weight_to=? WHERE id=?",
                    parameters)
                conn.commit()
                ...
                cursor.execute(
                    "UPDATE massogi SET users = ? WHERE id = ?", (users, cat_id,))
                conn.commit()
            else:
                parameters += (self.current_id,)
                cursor.execute(
                    "INSERT INTO massogi (gender,age_form,age_to,category_from,category_to,weight_from,weight_to,competition_id) VALUES (?,?,?,?,?,?,?,?)",
                    parameters)
                conn.commit()
                cursor.execute("SELECT MAX(id) FROM massogi")
    ...
    def delete_category(self, cat_id):
        ...
        if answer:
            with sqlite3.connect(DB_FILE) as conn:
                cursor = conn.cursor()
                cursor.execute("DELETE FROM massogi WHERE id = ?", (cat_id,))
                conn.commit()
            self.update_tables()
    ...
    def load_category_table(self):
        ...
        with sqlite3.connect(DB_FILE) as conn:
            ...
            cursor.execute("SELECT * FROM massogi WHERE competition_id = ?", (self.current_id,))
            massogi_data = cursor.fetchall()
            for num, row in enumerate(massogi_data):
    ...
    def shuffle(self):
        try:
            ...
            for i in cat_data:
                name = (f"Массоги {i[3]}-{i[4]}лет,"
                        f" {self.categories[i[5]]}-{self.categories[i[6]]},"
                        f"{i[7]}-{i[8]}кг, {i[2]}")
                users = json.loads(i[-1])
                if users:
                    cursor.execute(
                        f"SELECT * FROM athletes WHERE id IN ({','.join('?' for _ in users)}) AND massogi = ?",
                        users + ['да'])
                    ...
                    create_grid(users=users, cat_name=name, comp_name=comp_name, city=city, members=members)
            messagebox.showinfo("Готово", f'Сетки сохранены в "Массоги {comp_name}.xlsx"')
        except Exception as e:
